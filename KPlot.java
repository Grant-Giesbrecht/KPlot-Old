...
class KPlot{
    ...
    public KPlot(String window_name){
    	this(window_name, 500, 470);
    }

    public KPlot(String window_name, int x_size, int y_size){
        
        frame = new JFrame(window_name);
        frame.setSize(x_size, y_size); //X, Y
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLayout(new BorderLayout());
        
		sharedData = new KPlotSharedData();
        sharedData.x_min = -10;
        sharedData.y_min = -10;
        sharedData.x_max = 10;
        sharedData.y_max = 10;
		...
		panel = new GraphArea(sharedData);
    ...
    public void drawInGraphAxes(boolean v){
    	panel.drawInGraphAxes(v);
    }
    
    public void gridDashed(boolean v){
    	panel.setGridDashed(v);
    }
    
    public void setBounds(int x_min, int x_max, int y_min, int y_max){
    	sharedData.x_min = x_min;
    	sharedData.x_max = x_max;
    	sharedData.y_min = y_min;
    	sharedData.y_max = y_max;
    }
